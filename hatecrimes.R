...
# To set up the data, I seperated the national individual records of data and place into counts of incidents per month in excell by using the COUNTIF function for all unique month + year combinations. 
# I did this again for just the New Jersery data, a state with a more complete record of hate crime data collection. 
...
p
p + aes(y = 300) # make it warming stripes
p + aes(x = as.factor(Month), y = 300) # to avoid missing stripes, make yearmonth a factor 

# Add add minimal theme, legend, titles, axis labels. 
...
# Add add minimal theme, legend, titles, axis labels. Ideally, this section would also include a way to label the year on the x axis. 
...
#Tried to remove the unnessicary axises. Unknown why national graph is on 0.5 month increments and why the NJ is correctly on 1 month incriments 
